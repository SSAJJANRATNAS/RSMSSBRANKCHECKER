import sqlite3 import os from telegram import Update from telegram.ext import ApplicationBuilder, CommandHandler, MessageHandler, filters, ContextTypes

Bot credentials

TOKEN = "8011614651:AAHIclU4a27-NqRQUm4RD9c1C5z07u1GZGs" CHANNEL_USERNAME = "@l1_factory_Academy"

List of split database files

DB_FILES = [f"students_{i}.db" for i in range(1, 11)]

Get student result from multiple databases

def get_student_info(roll_number): for db_file in DB_FILES: if not os.path.exists(db_file): continue
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text( "👋 Welcome to RSMSSB Result Checker Bot.\n\nPlease send your Roll Number to check your result.", parse_mode="Markdown" )

Handle roll number messages

async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.effective_user.id
...
🆔 Roll Number       : {info['roll_number']} 🏅 All India Rank    : {info['rank']} 📂 Category Applied  : {info['applied_category']} 🧬 Gender            : {info['gender']} 🌍 Area              : {info['area']} 🎯 Category Rank     : {info['category_rank']} 🎖️ Selection Status  : {info['selection_status']} 🏷️ Selection Category: {info['selection_category']}

📢 Channel: L1 Factory Academy 🤖 Bot: @RSMSSBResultChecker_bot """ await update.message.reply_text(response, parse_mode="Markdown")

Bot setup

app = ApplicationBuilder().token(TOKEN).build() app.add_handler(CommandHandler("start", start)) app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
